...
* **리스크 관리 기능:** 주문 시 손절매(Stop Loss), 이익 실현(Take Profit) 주문을 동시에 설정하여 리스크를 관리하는 OCO(One Cancels the Other) 주문 로직을 추가하여 실제 자금을 사용하는 자동 트레이딩 코드 제작.
* **백테스팅 (Backtesting):** 과거 데이터를 기반으로 현재 개발한 전략의 수익률과 위험도를 미리 측정하는 백테스팅 기능을 추가하여 전략의 객관적인 성능을 검증할 예정입니다.
